...
public class DatasetPreparer
{
  ...
  {
    Collection<Double> controversyScores = getControversyScores(billIdFilename);

    saveData(controversyScores, outputFilename);
    System.out
        .println("Created the controversy score dataset: " + outputFilename);
  }
  ...
  {
    createWordCountDataset(purpose, "Lct", vocabularyMap, WordCountFormat.LCT);
  }
      ...
      throws IOException
  {
    ...
    System.out.println(
        "Created " + type + " dataset for " + purpose + " (Word count = "
            + totalWordCount + ", document count = " + documentCount + ")");
  }
      ...
      WordCountFormat wordCountFormat)
  {
    ...
    for (String word : wordCountMap.keySet())
    {
      ...
      {
        if(first == false)
          buffer.append(wordCountFormat == WordCountFormat.LCT ? ' ' : ',');
        else
          first = false;
      ...
      String filename, String columnSeparatorRegEx) throws IOException
  {
    ...
    for (String token : tokens)
    {
      if(token != null && !token.isEmpty())
      {
        Collection<String> columns = getColumns(token, columnSeparatorRegEx);
  ...
  {
    assert (DATASET_PERSISTENCE_DIRECTORY != null);
    Utility.saveData(DATASET_PERSISTENCE_DIRECTORY, filename, content, append);
  }
