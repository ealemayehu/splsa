...
public class Model
{
  ...
  private Model(List<List<WordInfo>> documents, Map<String, Integer> vocabulary,
      List<Double> controversyScores, double lambda, int K, double eta,
      Matrix beta, Matrix v, Purpose purpose) throws IOException
  {
  ...
  private void optimizeVLinearRegression()
  {
    ...
    LinearRegression linearRegression = new LinearRegression(thetaWithBias, c,
        ridge);
  ...
  private void optimizeVGradientAscent()
  {
    ...
    log("V: Average partial derivative: "
        + totalPartialDerivative / (K * MAX_V_ITERATIONS));
  ...
  {
    Matrix thetaWithBiasSquared = new Matrix(thetaWithBias.getRowDimension(),
        thetaWithBias.getColumnDimension());
  ...
  private void optimizeTheta()
  {
    ...
    System.out.print('\n');
    log("Theta: Computed partial derivative count: "
        + K * N * MAX_THETA_ITERATIONS);
    log("Theta: Average partial derivative: "
        + totalPartialDerivative / (K * N * MAX_THETA_ITERATIONS));
  ...
  private double partialDerivativeLikelihoodWrtThetaLinearModel(int d, int k)
  {
    ...
    secondTerm = lambda / N * (cachedThetaTimesV(d) - c.get(d, 0))
        * v.get(k, 0);
  ...
  private double partialDerivativeLikelihoodWrtThetaQuadraticModel(int d, int k)
  {
    ...
    if(purpose == Purpose.TRAINING)
      secondTerm = lambda / N * 2 * (cachedThetaTimesVSquared(d) - c.get(d, 0))
          * theta.get(d, k) * v.get(k, 0);
  ...
  private double likelihood() throws IOException
  {
    ...
    if(purpose == Purpose.TRAINING)
    {
      ...
      log("Topic likelihood = " + tcf + ", Controversy score likelihood = "
          + cscf);
      log("Product 1 = " + (1 - lambda) * tcf + ", Product 2 = " + lambda * cscf
          + ", LAMBDA = " + lambda);
    }
    else
    {
  ...
  {
    System.out
        .print("[" + new Date() + "]: " + message + (newLine ? '\n' : ""));
    System.out.flush();
  }
  ...
  private void printStochasticMatrixInfo(String tag, Matrix matrix)
  {
    ...
    log("Stochastic matrix info for " + tag + ": Invalid count = "
        + invalidCount + ", " + "Average sum = " + averageSum + ", " + "Max: "
  ...
  private List<Pair<Integer, Double>> sortStochasticMatrixRow(Matrix matrix,
      int row)
  {
  ...
  private void saveTopWords(String suffix) throws IOException
  {
    ...
    for (int k = 0; k < K; k++)
    {
      List<Pair<String, Double>> topWords = getTopWordsByTopic(words, beta, k,
          40);
  ...
  public String getOutputFilename(String prefix, String suffix, String purpose,
      double lambda, int k, double eta)
  {
